...
pub fn fillText(self: *OffscreenCanvasRenderingContext2D, text: []const u8, x: f64, y: f64, max_width: ?f64, exec: *Execution) !void {
    self.syncComposite();
...
pub fn strokeText(self: *OffscreenCanvasRenderingContext2D, text: []const u8, x: f64, y: f64, max_width: ?f64, exec: *Execution) !void {
    self.syncComposite();
    const owned = try exec.arena.dupe(u8, text);
