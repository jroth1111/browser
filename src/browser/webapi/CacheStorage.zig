...
pub fn has(_: *const CacheStorage, _: []const u8, exec: *const Execution) !js.Promise {
    return exec.js.local.?.resolvePromise(false);
}

pub fn open(_: *const CacheStorage, _: []const u8, exec: *const Execution) !js.Promise {
    const cache = try exec._factory.create(Cache{});
    return exec.js.local.?.resolvePromise(cache);
}

pub fn delete(_: *const CacheStorage, _: []const u8, exec: *const Execution) !js.Promise {
    return exec.js.local.?.resolvePromise(false);
}
